...
async function buildConversationResponse(convId: number, currentUserId: string) {
  ...
  return {
    ...
    lastMessage: lastMessage
      ? {
          ...
          senderFirstName: lastMessage.senderFirstName ?? null,
          senderLastName: lastMessage.senderLastName ?? null,
          senderProfileImageUrl: lastMessage.senderProfileImageUrl ?? null,
        }
      : null,
  };
}
...
router.get("/conversations", async (req, res): Promise<void> => {
  ...
  const results = await Promise.all(
    conversationIds.map((id) => buildConversationResponse(id, userId))
  );
...
router.post("/conversations", async (req, res): Promise<void> => {
  ...
  const result = await buildConversationResponse(conversation.id, currentUserId);
...
router.get("/conversations/:conversationId", async (req, res): Promise<void> => {
  ...
  const result = await buildConversationResponse(conversationId, userId);
  if (!result) {
...
router.get("/conversations/:conversationId/messages", async (req, res): Promise<void> => {
  ...
  res.json(
    messages.map((m) => ({
      ...
      senderFirstName: m.senderFirstName ?? null,
      senderLastName: m.senderLastName ?? null,
      senderProfileImageUrl: m.senderProfileImageUrl ?? null,
    }))
  );
});
